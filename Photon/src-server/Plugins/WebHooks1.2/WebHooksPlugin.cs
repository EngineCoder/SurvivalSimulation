...
namespace Photon.Hive.Plugin.WebHooks
{
    public class WebHooksPlugin : PluginBase
    {
        ...
        private const string GameBeforeJoinKey = "PathBeforeJoin";
...
//        private Dictionary<string, string> pluginConfig;
        ...
        private string gameBeforeJoinUrl = string.Empty;
        ...
        {
            if (string.IsNullOrEmpty(this.gameBeforeJoinUrl))
            {
                info.Continue();
            }
            else
...
//                this.pluginConfig = config;
                ...
                if (!string.IsNullOrEmpty(this.baseUrl))
                {
                    ...
                    this.gameBeforeJoinUrl = this.GetUrl(config, GameBeforeJoinKey);
                    ...
                    host.LogDebug($"BeforeJoin URL = {this.gameBeforeJoinUrl}");
...
//                    this.PluginHost.LogDebug("SetupInstance config:" + config.SerializeToString());
        ...
        private string GetUrl(Dictionary<string, string> config, string key)
        {
            ...
            if (!string.IsNullOrEmpty(setting))
            {
                ...
                if (setting.Length != 0)
                {
                    ...
                    url = url.Replace("{Region}", this.Region.Trim().Replace(" ", string.Empty));
        ...
        private void BeforeJoinCallback(IHttpResponse httpResponse, object userState)
        {
            IBeforeJoinGameCallInfo callInfo = httpResponse.CallInfo as IBeforeJoinGameCallInfo;
            if (this.TryGetForwardResponse(httpResponse, out WebhooksResponse response, out string errorMsg))
            {
                if (response.ResultCode == 0)
                {
                    callInfo.Continue();
                ...
                {
                    callInfo.Fail("User not authorized to join room.");
                    this.PluginHost.LogDebug($"User '{callInfo.UserId}' won't join room '{callInfo.Request.GameId}' server returned code:{response.ResultCode} message:'{response.Message}'");
                }
            }
            else
            {
                callInfo.Fail("User not authorized to join room.");
                this.PluginHost.LogDebug($"User '{callInfo.UserId}' won't join room '{callInfo.Request.GameId}', error:'{errorMsg}'.");
            }
        }
