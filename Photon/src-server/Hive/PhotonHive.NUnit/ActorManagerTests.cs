    ...
    public class ActorManagerTests
    {
        ...
        public void ActorsGetActorsByNumbersOneDoesNotExistTest()
        {
            ...
            Assert.That(manager.TryAddPeerToGame(
                game, hivePeer1, 0, out actor, out errorCode, out reason, joinRequest), Is.True);

            joinRequest.ActorProperties[(byte)ActorParameter.UserId] = "a2";
            Assert.That(manager.TryAddPeerToGame(
                game, hivePeer2, 0, out actor, out errorCode, out reason, joinRequest), Is.True);

            joinRequest.ActorProperties[(byte)ActorParameter.UserId] = "a3";
            Assert.That(manager.TryAddPeerToGame(
                game, hivePeer3, 0, out actor, out errorCode, out reason, joinRequest), Is.True);

            joinRequest.ActorProperties[(byte)ActorParameter.UserId] = "a4";
            manager.TryAddPeerToGame(game, hivePeer4, 0, out actor, out errorCode, out reason, joinRequest);
