// --------------------------------------------------------------------------------------------------------------------
// <copyright file="HivePeer.cs" company="Exit Games GmbH">
...
{
    public interface IHivePeerLogContext : IPeerBaseLogContext
    {
        LogCountGuard UniqKeysValidationLogGuard { get; }
        LogCountGuard ValidateOpCountGuard {get;}

        LogCountGuard WebRpcLimitCountGuard { get; }

        LogCountGuard OperationParametersLogGuard { get; }
    }
    ...
    public class HivePeer : ClientPeer
    {
            ...
            : base(request)
        {
            ...
            this.SetLogContext(HiveLogContext.Default);
        }
        ...
        public PropertiesLimitsSettings PropertiesLimits { get; protected set; } = new PropertiesLimitsSettings();

        private IHivePeerLogContext PeerLogContext => (IHivePeerLogContext)this.logContext;
        ...
        public bool ValidateOperation(Operation operation, SendParameters sendParameters)
        {
            ...
            {
                Loggers.InvalidOpLogger.Info(this.PeerLogContext.ValidateOpCountGuard, $"Invalid operation: code_{operation.Code}. reason: '{errorMessage}', p:{this}");
            }
            return false;
        }
        ...
        protected virtual void HandleCreateGameOperation(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            if (!createRequest.ValidateParametersLimit(this.PropertiesLimits, out var errorMsg))
            {
                if (log.IsWarnEnabled)
                {
                    log.WarnFormat(this.PeerLogContext.OperationParametersLogGuard,
                        "Limit exceeded during create game. errorMsg:{0}, p:{1}", errorMsg, this);
                }

                if (!this.PrivateInboundController.OnlyLogLimitsViolations)
                {
                    this.SendOperationResponseAndDisconnect(new OperationResponse((byte)OperationCode.CreateGame)
                    {
                        DebugMessage = errorMsg,
                        ReturnCode = ErrorCodes.OperationParametersLimitExceeded,
                    }, sendParameters);

                    if (Loggers.DisconnectLogger.IsInfoEnabled)
                    {
                        Loggers.DisconnectLogger.InfoFormat(this.PeerLogContext.DisconnectLogGuard,
                            "Disconnect peer Limit exceeded during join game. Reason:{0}, Msg:{1}, p:{2}",
            ...
            if (!this.MayJoinOrCreateGame(createRequest, out var returnCode, out var debugMsg))
            {
                ...
                if (Loggers.DisconnectLogger.IsInfoEnabled)
                {
                    Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, 
                        $"Disconnect peer may not create game. Reason_{returnCode}, Msg:{debugMsg}, p:{this}");
                }
                return;
            }
        ...
        protected virtual void HandleSetPropertiesOperation(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            {
                log.Warn(this.PeerLogContext.UniqKeysValidationLogGuard, "Uniq Keys Validation: peer set " +
                                                                          $"too many uniq properties. limit={this.PropertiesLimits.MaxUniqPropertyKeysPerPeer}. already set={this.peerUniqPropertyKeys.Count}. " +
                ...
                if (!this.PrivateInboundController.OnlyLogLimitsViolations)
                {
                    ...
                    if (Loggers.DisconnectLogger.IsInfoEnabled)
                    {
                        Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, $"Disconnect uniq properties limit. Reason_{ErrorCode.OperationLimitReached}, Msg:{ErrorMessages.TooManyPropertiesSetByPeer}, p:{this}");
                    }
                    return;
                }
            }
        ...
        protected virtual void HandleJoinGameOperation(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            if (!joinRequest.ValidateParametersLimit(this.PropertiesLimits, out var errorMsg))
            {
                if (log.IsWarnEnabled)
                {
                    log.WarnFormat(this.PeerLogContext.OperationParametersLogGuard,
                        "Limit exceeded during join game. errorMsg:{0}, p:{1}", errorMsg, this);
                }
            ...
            if (!this.MayJoinOrCreateGame(joinRequest, out var returnCode, out var debugMsg))
            {
                ...
                if (Loggers.DisconnectLogger.IsInfoEnabled)
                {
                    Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, $"Disconnect peer may not join game. Reason_{returnCode}, Msg:{debugMsg}, p:{this}");
                }
        ...
        protected virtual void HandleLeaveOperation(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            {
                Loggers.DisconnectLogger.InfoFormat(this.PeerLogContext.DisconnectLogGuard, 
                    "Disconnect because of leave. Reason: ErrorCode.LeaveRequest. p:{0}", this);
            }
        }
            ...
            if (this.WebRpcHandler != null)
            {
                ...
                if (callsCount > this.HttpRpcCallsLimit)
                {
                    ...
                    else
                    {
                        ...
                        if (Loggers.DisconnectLogger.IsInfoEnabled)
                        {
                            Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, "Disconnect HandleRpcOperation. " +
                                $"Reason_{resp.ReturnCode}, Msg:{resp.DebugMessage}");
                        }
                    }

                    log.Warn(this.PeerLogContext.WebRpcLimitCountGuard, $"Limit exceeded Too many web rpc requests. limit: {this.HttpRpcCallsLimit} callsCount_{callsCount}");
                    return;
                }
        ...
        protected override void OnDisconnect(int reasonCode, string reasonDetail)
        {
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("OnDisconnect: conId={0}, lastErrorCode_{3}, reason={1}, reasonDetail={2}", this.ConnectionId, reasonCode, reasonDetail, this.LastErrorCode);
            }
        ...
        private void HandleRoomNotFound(SendParameters sendParameters, JoinGameRequest joinRequest)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("Game '{0}' userId '{1}' failed to join. msg:{2} -- p:{3}", joinRequest.GameId, this.UserId,
        ...
        private void OnWrongOperationStage(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            {
                Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, 
                    $"Disconnect wrong operation stage. Reason_{ErrorCode.OperationDenied}, Msg:{HiveErrorMessages.OperationIsNotAllowedOnThisJoinStage}, p:{this}");
            }
        }
        ...
        private bool ValidateUniqKeys(Hashtable properties)
        {
            ...
            var limit = this.PropertiesLimits.MaxUniqPropertyKeysPerPeer;
        ...
        protected override void OnSendBufferFull()
        {
            ...
            {
                Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, 
                    $"Disconnect send buffer full, p:{this}");
            }
        ...
        protected override void OnSendBufferEmpty()
        {
            ...
            {
                Loggers.DisconnectLogger.Info(this.PeerLogContext.DisconnectLogGuard, 
                    $"Disconnect send buffer full from OnSendBufferEmpty, p:{this}");
            }
        }
