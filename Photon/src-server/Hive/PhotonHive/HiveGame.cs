...
namespace Photon.Hive
{
    ...
    public abstract class HiveGame : Room
    {
        ...
        protected readonly PropertiesLimitsSettings propertiesLimits;

        protected readonly IHiveLogContext logContext;

        //cleanup after
        protected int restoredByPlugin;
        #endregion
        ...
        /// <param name="propLimits">limits that we apply to properties</param>
        /// <param name="hiveLogContext">contains stuff that helps us log in more predictable way</param>
        protected HiveGame(string gameName, RoomCacheBase roomCache, IGameStateFactory gameStateFactory = null, 
            int maxEmptyRoomTTL = 0, ExtendedPoolFiber executionFiber = null, PropertiesLimitsSettings propLimits = null, 
            IHiveLogContext hiveLogContext = null)
            : base(gameName, executionFiber, roomCache, gameStateFactory, maxEmptyRoomTTL)
        {
            ...
            this.propertiesLimits = propLimits ?? new PropertiesLimitsSettings();

            this.logContext = hiveLogContext ?? HiveLogContext.Default;
        }
        ...
        public override string ToString()
        {
            ...
            try
            {
                value =
                    ...
                    $"Active={this.ActiveActors.Count()} Inactive={this.InactiveActors.Count()} IsClosed={this.IsFinished} UtcCreated={this.UtcCreated:yyyy-MM-dd--HH:mm:ss.fff} RemovePath={this.RemoveRoomPathString}," +
                    $"RestoredByPlugin={this.restoredByPlugin},CheckUserOnJoin:{this.CheckUserOnJoin}";
...
#region Methods
        ...
        private bool CheckThisIsJustCreatedRoom()
        {
            return this.ActorsManager.ActorNumberCounter == 0;
        }
        ...
        protected virtual void CleanupActor(Actor actor)
        {
            ...
            actor.RemoveGroups(Array.Empty<byte>());
        ...
        protected override void ExecuteOperation(HivePeer peer, Operation operation, SendParameters sendParameters)
        {
                    ...
                    default:
                        {
                            ...
                            {
                                Loggers.DisconnectLogger.Info(this.logContext.GameInvalidOpLogGuard,
                                    $"Disconnect from ExecuteOperation operation_{(OperationCode)operation.Code},  p:{this}");
                            }
                            ...
                            if (Loggers.DisconnectLogger.IsInfoEnabled)
                            {
                                Loggers.DisconnectLogger.Info(this.logContext.GameDisconnectLogGuard,
                                    $"Disconnect from ExecuteOperation Reason_{ErrorCode.OperationInvalid}, " +
                                    $"Msg: {message}, p:{this}");
                            }
            ...
            {
                if (this.logContext.ExceptionLogGuard.IncrementAndCheck())
                {
                    var message = LogExtensions.AddSkippedMessagesInfo(this.logContext.ExceptionLogGuard,
        ...
        protected virtual void JoinFailureHandler(HivePeer peer, JoinGameRequest request)
        {
            if (Log.IsDebugEnabled)
            {
        ...
        {
            var isCreatingGame = peer.JoinStage == HivePeer.JoinStages.CreatingOrLoadingGame;
            var isNewGame = false;
            if (isCreatingGame)
            {
                if (this.CheckThisIsJustCreatedRoom())
                {
                    isNewGame = true;
            ...
            {
                var msg = string.Format(HiveErrorMessages.CacheSliceNoAvailable, joinRequest.CacheSlice);
            ...
            // create an new actor
            if (this.TryAddPeerToGame(peer, joinRequest.ActorNr, out actor, out var errorCode, out var reason, joinRequest) == false)
            {
        ...
        {            
                ...
                && !this.CacheDiscarded)
            {
                ...
                {
                    Log.WarnFormat(this.logContext.ForceGameCloseLogGuard, "Game forced to close. cache limit exceeded. ActorsCacheDiscarded={0}, EventsCacheDiscarded:{1}, Room:{2}",
        ...
        protected bool SetProperties(int actorNr, Hashtable properties, Hashtable expected, bool broadcast)
        {
            ...
            if (!LobbyPropertiesValidator.ValidateLimits(request, this.Properties, this.propertiesLimits, out errorMsg))
            {
                Log.WarnFormat(this.logContext.LobbyPropertiesValidatorLogGuard,
                    "Limit exceeded: SetProperties from plugin. errorMsg:{0}, game:{1}", errorMsg, this.Name);
        ...
        protected bool ValidateAndFillSetPropertiesRequest(HivePeer peer, SetPropertiesRequest request, out string errorMsg)
        {
            ...
            var isGamePropertiesUpdate = request.UpdatingGameProperties;
        ...
        private bool GenericCheckBeforeApplyProperties(SetPropertiesRequest request, out string errorMessage)
        {
            ...
            bool requestMayChangeExpectedUsers = request.ExpectedValues != null && request.ExpectedValues.ContainsKey((byte)GameParameter.ExpectedUsers);
            if (request.ExpectedUsers != null && !requestMayChangeExpectedUsers)
            {
                if (request.ExpectedValues != null)// we need this check only for logging
                {
                    Log.Warn(this.logContext.NoExpectedUsersLogGuard, "CAS is used but expectedUsers are not there");
                }
            ...
            if (request.newMaxPlayer.HasValue && request.newMaxPlayer.Value != 0)
            {
                ...
                if (!requestMayChangeExpectedUsers)
                {
        ...
        protected void CheckTotalPropertiesSize(SetPropertiesRequest setPropertiesRequest)
        {
            ...
            {
                Log.Warn(this.logContext.ForceGameCloseLogGuard, 
        ...
        {
            return this.ActorsManager.TryAddPeerToGame(this, peer, actorNr, out actor, out errorCode, out reason, joinRequest);
        }
