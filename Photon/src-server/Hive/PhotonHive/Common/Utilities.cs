...
namespace Photon.Hive.Common
{
    ...
    {
        public static readonly string amf3IsVisiblePropertyKey = ((byte)GameParameter.IsVisible).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3IsOpenPropertyKey = ((byte)GameParameter.IsOpen).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3MaxPlayerPropertyKey = ((byte)GameParameter.MaxPlayers).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3PropertiesPropertyKey = ((byte)GameParameter.LobbyProperties).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3MasterClientIdPropertyKey = ((byte)GameParameter.MasterClientId).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3ExpectedUsersPropertyKey = ((byte)GameParameter.ExpectedUsers).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3PlayerTTLPropertyKey = ((byte)GameParameter.PlayerTTL).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3EmptyRoomTTLPropertyKey = ((byte)GameParameter.EmptyRoomTTL).ToString(CultureInfo.InvariantCulture);

        public static readonly string amf3NicknamePropertyKey = ((byte)ActorParameter.Nickname).ToString(CultureInfo.InvariantCulture);

        /// <summary>
        /// Converts well known properties sent by AS3/Flash clients - from string to byte-keys.
        /// </summary>
        /// <remarks>
        /// Check if peer is a flash (amf3) client because flash clients does not support byte keys in a hashtable. 
        ...
        /// <param name="gamePropsMeta">meta data for gameProps</param>
        /// <param name="actorPropsMeta">meta data for actorProps</param>
        public static void ConvertAs3WellKnownPropertyKeys(Hashtable gameProps, Hashtable actorProps, ParameterMetaData gamePropsMeta, ParameterMetaData actorPropsMeta)
        {
            ...
            if (gameProps != null && gameProps.Count > 0)
            {
                ...
                UpdatePropertyKeyType(gameProps, gamePropsMeta, (byte)GameParameter.IsVisible, amf3IsVisiblePropertyKey);
        ...
        public static void ConvertAs3WellKnownPropertyKeys(IList gamePropertyKeys, IList actorPropertyKeys)
        {
            ...
            {
                // well known property "is visible"
                UpdateKeyTypeInList(gamePropertyKeys, (byte)GameParameter.IsVisible, amf3IsVisiblePropertyKey);
