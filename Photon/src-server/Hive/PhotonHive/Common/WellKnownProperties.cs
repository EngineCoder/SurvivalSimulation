...
namespace Photon.Hive.Common
{
    public class WellKnownProperties
    {
            ...
            if (GameParameterReader.TryReadGameParameter(propertyTable, GameParameter.ExpectedUsers, out value, paramMetaData))
            {
                if (value != null)
                {
                    if (value is string[] == false
                        ...
                        && !ConvertToStringArray(out value, objArray))
                    {
        ...
        private static bool ConvertToStringArray(out object value, IReadOnlyList<object> objArray)
        {
            var arr = new string[objArray.Count];
        ...
        public static bool UpdateNumericWellKnownPropertiesTypes(Hashtable propertyTable, out string errorMsg, ParameterMetaData paramMetaData)
        {
            if (!GameParameterReader.TryReadByteParameter(propertyTable, GameParameter.MaxPlayers, out _, out var value, paramMetaData))
            {
                errorMsg = GetInvalidGamePropertyTypeMessage(GameParameter.MaxPlayers, typeof(byte), value);
                return false;
            }

            if (!GameParameterReader.TryReadIntParameter(propertyTable, GameParameter.MasterClientId, out _, out value, paramMetaData))
            {
                errorMsg = GetInvalidGamePropertyTypeMessage(GameParameter.MasterClientId, typeof(int), value);
                return false;
            }

            if (!GameParameterReader.TryReadIntParameter(propertyTable, GameParameter.PlayerTTL, out _, out value, paramMetaData))
            {
                errorMsg = GetInvalidGamePropertyTypeMessage(GameParameter.PlayerTTL, typeof(int), value);
                return false;
            }

            if (!GameParameterReader.TryReadIntParameter(propertyTable, GameParameter.EmptyRoomTTL, out _, out value, paramMetaData))
            {
                errorMsg = GetInvalidGamePropertyTypeMessage(GameParameter.EmptyRoomTTL, typeof(int), value);
                return false;
            }
