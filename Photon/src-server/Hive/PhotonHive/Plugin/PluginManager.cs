...
namespace Photon.Hive.Plugin
{
    ...
    public class PluginTraits
    {
        ...
        public bool IsPersistent => this.isPersistent;

        public bool AsyncJoin => this.asyncJoin;

        public bool AllowAsyncJoin => this.IsPersistent && this.AsyncJoin;
    ...
    public class PluginManager : IPluginManager
    {
        ...
        public PluginManager(string basePath)
        {
            try
            {
                ...
                else
                {
                    ...
                    this.initErrorMsg = "Plugins usage is disabled";
                }
            }
            catch (Exception e)
            {
        ...
        public PluginTraits PluginTraits => this.pluginTraits;
        ...
        public virtual IPluginInstance GetGamePlugin(IPluginHost sink, string pluginName)
        {
...
#endif
            {
                ...
                return LoadErrorPlugin(sink, $"PluginManager initialization failed.{this.initErrorMsg}");
            }
            ...
            catch (Exception e)
            {
                ...
                Log.Error(errorMsg);
                return LoadErrorPlugin(sink, errorMsg);
            }
        ...
        public static IPluginInstance GetDefaultPlugin(IPluginHost sink)
        {
            ...
            plugin.SetupInstance(sink, null, out _);
        ...
        private bool SetupPluginManager(string path, string typeName, Dictionary<string, string> config)
        {
            ...
            try
            {
                ...
                {
                    this.initErrorMsg = "Can not load plugin assembly. It does not refer PhotonHivePlugin.dll";
                ...
                {
                    this.initErrorMsg = "Plugins version mismatch";
                    Log.ErrorFormat("Plugins version mismatch. PhotonHivePlugin Version:{0}; Plugin Name:{1} Version:{2}", currentPluginsVersion, asm.GetName().Name, pluginVersion);
                    return false;
                }
                ...
                if (this.pluginFactory == null)
                {
                    this.initErrorMsg = "Factory type does not implement IPluginFactory interface";
            ...
            {
                this.initErrorMsg = "got exception during either plugins assembly or plugins type loading";
                Log.Error($"got exception during either loading of assembly {path} or type {typeName}. Exception Msg: {e.Message}", e);
            }
            return false;
        }
        ...
        private void CreateKnownType(string typeName, Assembly asm)
        {
            ...
            Log.InfoFormat("Plugin Type {0} from assembly {1} was successfully created", typeName, asm.FullName);
        }
        ...
        {
            var plugin = this.pluginFactory.Create(sink, pluginName, this.pluginConfig, out var errorMsg);
            if (plugin == null)
            {
                return LoadErrorPlugin(sink, errorMsg).Plugin;
            }
        ...
        private static IPluginInstance LoadErrorPlugin(IPluginHost sink, string errorMsg)
        {
