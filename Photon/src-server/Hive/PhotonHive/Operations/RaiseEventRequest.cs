...
namespace Photon.Hive.Operations
{
    ...
    public class RaiseEventRequest : Operation, IRaiseEventRequest
    {
        ...
        public bool IsCacheOpRemoveFromCache => this.Cache == (byte)CacheOperation.RemoveFromRoomCache;

        public bool IsCacheOpRemoveFromCacheForActorsLeft => this.Cache == (byte)CacheOperation.RemoveFromCacheForActorsLeft;

        public bool IsCacheSliceIndexOperation => this.Cache >= (byte)CacheOperation.SliceIncreaseIndex;

        public bool IsCacheOnlyOperation =>
            this.IsCacheSliceIndexOperation
            || this.IsCacheOpRemoveFromCache
            || this.IsCacheOpRemoveFromCacheForActorsLeft;

        public bool IsBroadcastOperation => !this.IsCacheOnlyOperation;
        ...
        public bool SimulatesPhotonEvent => this.EvCode > (byte) EventCode.LastPhotonEvent;

        public byte OperationCode => this.OperationRequest.OperationCode;

        public System.Collections.Generic.Dictionary<byte, object> Parameters => this.OperationRequest.Parameters;
        ...
        public void ValidateActorsList(int currentActorsCount)
        {
            if (this.Actors != null && this.Actors.Length > 0)
            {
                ...
                {
                    this.isValid = false;
                    this.errorMessage = "Actors exceeds CurrentPlayers count";
                    return;
                }
        ...
        {
            if (!Enum.IsDefined(typeof(CacheOperation), (int)this.Cache))
            {
                this.isValid = false;
                this.errorMessage = $"Non existing CachingOption value {this.Cache}.";
                return false;
            }

            if (this.Cache > (byte)CacheOperation.SliceIncreaseIndex &&
                this.Cache <= (byte)CacheOperation.SlicePurgeUpToIndex)
            {
                if (!this.CacheSliceIndex.HasValue)
                {
                    this.isValid = false;
                    this.errorMessage = "slice index is not set ";
                    return false;
                }
            }

            // validate Receiver group
            if (this.ReceiverGroup > 2)
            {
                this.isValid = false;
                this.errorMessage = $"Non existing receiver group: {this.ReceiverGroup}. should be 0, 1 or 2";
                return false;
            }
            ...
            try
            {
                this.WebFlags = Convert.ToByte(value);
            }
            catch (Exception e)
            {
                this.isValid = false;
