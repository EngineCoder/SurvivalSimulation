...
namespace Photon.Hive.Operations
{
    ...
    public class JoinGameRequest : Operation, IJoinGameRequest
    {
        ...
        public bool ValidateParametersLimit(PropertiesLimitsSettings propertiesLimits, out string errorMsg)
        {
            if (!this.ValidatePropertiesSize(propertiesLimits.MaxPropertiesSizePerGame, out errorMsg))
            {
                return false;
            }

            //validate lobby properties

            if (!LobbyPropertiesValidator.ValidateLimits(this, propertiesLimits, out errorMsg))
            {
                return false;
            }
        ...
        public bool ValidatePropertiesSize(int propertiesSizeLimit, out string errorMsg)
        {
            var requestMetaData = this.RequestMetaData;
            ...
            var totalPropsSize = 0;
            if (gamePropsMeta != null)
            {
                totalPropsSize += gamePropsMeta.DataSize;
            }

            totalPropsSize += actorPropsMeta?.DataSize ?? 0;

            if (totalPropsSize > propertiesSizeLimit)
            {
                this.isValid = false;
                errorMsg = "Properties size is too big";
                return true;
            }
