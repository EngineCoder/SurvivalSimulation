...
namespace Photon.UnitTest.Utils.Basic.NUnitClients
{
    public class OfflineNUnitClient : UnitTestClient, INUnitClient
    {
        ...
        private ConnectPolicy Policy { get; }
        public Dictionary<long, object> OnMessageBuffer { get; set; }

        public string RemoteEndPoint => this.ServerAppProxy.EndPoint.ToString();
        ...
        public bool SendRequest(OperationRequest op, bool encrypted)
        {
            ...
            {
                r.Parameters = (Dictionary<byte, object>)op.Parameters;
            }
        ...
        {
            Thread.Sleep(40);
            var res = this.EventQueue.DequeueIf(data => data.Code == eventCode, millisecodsWaitTime);
        ...
        protected override void OnMessage(object message)
        {
            ...
            this.OnMessageBuffer.Add(DateTime.Now.Ticks, message);
         }

        public NetworkProtocolType NetworkProtocol => NetworkProtocolType.Tcp;

        public void SetupEncryption(Dictionary<byte, object> encryptionData)
        {
            var edc = new EncryptionData(this.Protocol, encryptionData);
            this.CryptoProvider = new RijndaelCryptoProvider(edc.EncryptionSecret, PaddingMode.PKCS7);
        }
    }
}
