...
namespace Photon.UnitTest.Utils.Basic
{
    ...
    public class PhotonStarter
    {
        ...
        public static Process Start(string serverAddress, string appId, ConnectionProtocol protocol, string photonCmdLine)
        {
            ...
            string defaultRelativeWorkingDirectory = FindConfig("dev_out");
            if (defaultRelativeWorkingDirectory != null) defaultRelativeWorkingDirectory = Path.Combine(defaultRelativeWorkingDirectory,"dev_out", "framework","config");
        ...
        private static string FindClassicPhoton()
        {
            ...
            var index = dirs.TakeWhile(dir => !dir.StartsWith("src-")).Count();
        ...
        {
            var dirs = Environment.CurrentDirectory.Split(Path.DirectorySeparatorChar);
            var index = dirs.TakeWhile(dir => dir.StartsWith("src-") == false).Count();
            if (index == dirs.Length)
            {
                throw new Exception(string.Format("Can't find 'src-*' in folder '{0}'", Environment.CurrentDirectory));
            }
        ...
        private static string FindConfig(string subPath)
        {
            //var subPathParts = subPath.Split(Path.PathSeparator);

            var currentDir = Environment.CurrentDirectory;
            var relPath = ""+Path.DirectorySeparatorChar;

            while (Directory.Exists(Path.Combine(currentDir, subPath)) == false)
            {
                currentDir = Path.GetFullPath(Path.Combine(currentDir, ".."));
                relPath += ".." + Path.DirectorySeparatorChar;
                if (Path.GetPathRoot(currentDir) == currentDir)
                {
                    return null;
                }
            }
