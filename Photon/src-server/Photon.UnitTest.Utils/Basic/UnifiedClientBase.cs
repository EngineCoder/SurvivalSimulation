...
namespace Photon.UnitTest.Utils.Basic
{
    public abstract class UnifiedClientBase : IDisposable
    {
        ...
        public string RemoteEndPoint => this.Client.RemoteEndPoint;
        ...
        public bool WaitForDisconnect(int millisecondsWaitTime = ConnectPolicy.WaitTime)
        {
            if (this.Client != null)
            {
                return this.Client.WaitForDisconnect(millisecondsWaitTime);
            }
        ...
        public bool WaitForDisconnectMessage(int millisecondsWaitTime = ConnectPolicy.WaitTime)
        {
            if (this.Client != null)
            {
                return this.Client.WaitForDisconnectMessage(millisecondsWaitTime);
            }

            throw new Exception("Client is not set");
        }

        public EventData WaitForEvent(int millisecondsWaitTime = ConnectPolicy.WaitTime)
        {
            if (this.Client != null)
            {
                return this.Client.WaitForEvent(millisecondsWaitTime);
            }
            throw new Exception("Client is not set");
        }
