...
using Settings = Photon.Common.Authentication.Settings;

namespace Photon.LoadBalancing.GameServer
{
    public interface IGameClientPeerLogContext : IHivePeerLogContext 
    {
        LogCountGuard SecureConnectionLogGuard { get; }
    }
    ...
    public class GameClientPeer : HivePeer, IAuthTimeoutCheckerClient
    {
        ...
        protected readonly LBTokenValidator tokenValidator;

        protected int NumberAuthRequests;
        protected int MaxNumberAuthRequests = 3;

        protected delegate bool ValidateAuthTokenDelegate(AuthenticateRequest authenticateRequest, SendParameters sendParameters,
        ...
        /// </summary>
        /// <param name="initRequest">init request that application gets during CreatePeer call</param>
        /// <param name="application">photon application</param>
        /// <param name="tokenValidator"></param>
        public GameClientPeer(InitRequest initRequest, GameApplication application, LBTokenValidator tokenValidator)
            : this(initRequest, application, tokenValidator, false)
        {
            application.AppStatsPublisher?.IncrementPeerCount();
        }

        protected GameClientPeer(InitRequest initRequest, GameApplication application, LBTokenValidator tokenValidator, bool derived)
            : base(initRequest)
        {
            this.application = application;
            this.PropertiesLimits = GameServerSettings.Default.Limits.Inbound.Properties;
            ...
            this.SetLogContext(GSLogContext.Default);
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                if (authToken != null)
                {
                    if (this.tokenValidator.ValidateAuthToken(new AuthenticateRequest(), new SendParameters(), authToken, this, out var errCode, out errorMsg) == false)
                    {
                        errorCode = (short)errCode;
                        authToken = null;
                    }
            ...
            if (authToken != null)
            {
                ...
                if (!derived)
                {
                    ...
                    {
                        log.Warn(this.GameClientPeerLogContext.SecureConnectionLogGuard,
        ...
        protected bool ProcessValidateToken(ValidateAuthTokenDelegate validateAuthToken, AuthenticateRequest authenticateRequest, SendParameters sendParameters,
            AuthenticationToken authToken)
        {
            if (validateAuthToken.Invoke(authenticateRequest, sendParameters, authToken, out var errorCode, out string errorMsg) == false)
            {
                this.RequestFiber.Enqueue(() => this.SendOperationResponseAndDisconnect(new OperationResponse((byte)OperationCode.AuthOnce)
                {
                    DebugMessage = errorMsg,
        ...
        private IGameClientPeerLogContext GameClientPeerLogContext => (IGameClientPeerLogContext)this.logContext;
...
//            }
        ...
        protected override void OnOperationRequest(OperationRequest request, SendParameters sendParameters)
        {
            ...
            if (request.OperationCode == (byte) OperationCode.Authenticate)
            {
                if (this.IsAuthenticated)
                {
                    ...
                    {
                        Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                            $"Disconnect from OnOperationRequest Reason_{ErrorCode.OperationDenied}, Msg:{LBErrorMessages.AlreadyAuthenticated}, p:{this}");
                    }
            ...
            if (!this.IsAuthenticated)
            {
                ...
                {
                    Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                        $"Disconnect from OnOperationRequest Reason_{ErrorCode.OperationDenied}, Msg:{LBErrorMessages.NotAuthorized}, p:{this}");
                }
                return;
            }
        ...
        protected AuthenticationToken GetValidAuthToken(AuthenticateRequest authenticateRequest, SendParameters sendParameters)
        {
            if (this.application.TokenCreator == null)
            {
                ...
                {
                    Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                        $"Disconnect from GetValidAuthToken Reason_{ErrorCode.InvalidAuthentication}, Msg:{ErrorMessages.AuthTokenTypeNotSupported}, p:{this}");
                }
            ...
            if (string.IsNullOrEmpty(authenticateRequest.Token))
            {
                ...
                {
                    Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                        $"Disconnect from GetValidAuthToken Reason_{ErrorCode.InvalidAuthentication}, Msg:{ErrorMessages.AuthTokenMissing}, p:{this}");
                }
            ...
            if (!tc.DecryptAuthenticationToken(authenticateRequest.Token, out var authToken, out var errorMsg))
            {
                ...
                {
                    Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                        $"Disconnect from GetValidAuthToken Reason_{ErrorCode.InvalidAuthentication}, Msg:{ErrorMessages.AuthTokenInvalid}, p:{this}");
                }
                return null;
            }
            ...
            if (!this.tokenValidator.ValidateAuthToken(authenticateRequest, sendParameters, authToken, this, out var errorCode, out errorMsg))
            {
                this.SendOperationResponseAndDisconnect(
                    new OperationResponse(authenticateRequest.OperationRequest.OperationCode)
                    {
                ...
                {
                    Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                        $"Disconnect from GetValidAuthToken Reason_{ErrorCode.ExpectedGSCheckFailure}, Msg:{ErrorMessages.ExpectedGSCheckFailure}, p:{this}");
                }
                return null;
            }
        ...
        protected override void HandleCreateGameOperation(OperationRequest operationRequest, SendParameters sendParameters)
        {
            if (this.application.ServerState != ServerState.Normal)
            {
                ...
                {
                    Loggers.DisconnectLogger.Info(this.GameClientPeerLogContext.DisconnectLogGuard, 
                        $"Disconnect from HandleCreateGameOperation Reason_{ErrorCode.OperationDenied}, Msg:{LBErrorMessages.ServerClosedForGameCreation}, p:{this}");
                }
        ...
        private void SendAuthEvent()
        {
            ...
            var response = new EventData((byte) EventCode.AuthEvent)
            {
            ...
            {
                Encrypted = this.IsPayloadEncryptionEnabled
            };

            this.SendEvent(response, sendParameters);
        }
        ...
        protected override bool MayJoinOrCreateGame(JoinGameRequest joinGameRequest, out short errorCode, out string errorMsg)
        {
            ...
            if (!this.tokenValidator.ValidateExpectedGame(joinGameRequest, this, out var errCode, out errorMsg))
            {
