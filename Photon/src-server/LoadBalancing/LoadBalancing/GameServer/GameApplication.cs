...
using Photon.SocketServer.Configuration;
...
namespace Photon.LoadBalancing.GameServer
{
    public class GameApplication : ApplicationBase
    {
        ...
        private static IConfiguration LoadConfiguration()
        {
            ...
            var cbpath = Path.GetDirectoryName(typeof(GameApplication).Assembly.Location);
            var c = cb.AddXmlFile(Path.Combine(cbpath, "GameServer.xml.config")).Build();

            ExpandEnvironmentVariables(c);
        ...
        {
            var peer = new GameClientPeer(initRequest, this, new LBTokenValidator());
            {
                if (this.webRpcManager.IsRpcEnabled)
                {
                    peer.WebRpcHandler = this.webRpcManager.GetWebRpcHandler();
                }
                // use this if you need to send some response to your client
        ...
        protected override void Setup()
        {
            ...
            if (CommonSettings.Default.EnablePerformanceCounters)
            {
                ...
                LBPerformanceCounters.Initialize();
            }
            else
            {
        ...
        {
            var controller = this.GetInboundController();

            Protocol.InboundController = controller;
        }

        protected virtual InboundController GetInboundController()
        {
            var controller = new InboundController(
            ...
            controller.SetupOperationParameter((byte)Hive.Operations.OperationCode.RaiseEvent,
                (byte)Hive.Operations.ParameterKey.Data,
            ...
            controller.SetupOperationParameter((byte)Hive.Operations.OperationCode.JoinGame,
                (byte)Hive.Operations.ParameterKey.ActorProperties,
            ...
            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.SetProperties,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.SetPropertiesRate));
            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.GetProperties,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.GetPropertiesRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.JoinGame,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.JoinGameRate));
            ...
            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.CreateGame,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.CreateGameRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Leave,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.LeaveRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Ping,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.PingRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.ChangeGroups,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.ChangeGroupsRate));
            ...
            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.DebugGame,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.DebugGameRate));
            ...
            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Settings,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.SettingsRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.RaiseEvent,
                new OperationLimits(InboundController.Unlimited, GameServerSettings.Default.Limits.Inbound.Operations.RaiseEventRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Authenticate,
                new OperationLimits(InboundController.Unlimited,
                    GameServerSettings.Default.Limits.Inbound.Operations.AuthenticateRate));
            return controller;
        }
        ...
        private void SetupFeedbackControlSystem()
        {
            ...
            this.WorkloadController = new WorkloadController(
                this, "_Total", 1000, this.ServerId.ToString(), workLoadConfigFile, GameServerSettings.Default.MaxCcu);
        ...
        protected override void OnStopRequested()
        {
            ...
            {
                this.MasterServerConnection.SendLeaveEventAndWaitForDisconnect(GameServerSettings.Default.S2S.StopNotificationTimeout);
