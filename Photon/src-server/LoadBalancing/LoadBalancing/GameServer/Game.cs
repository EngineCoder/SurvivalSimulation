...
namespace Photon.LoadBalancing.GameServer
{
    public class Game : HiveHostGame
    {
        ...
        private readonly IGSLogContext gsLogContext;
        ...
        {
            this.gsLogContext = gameCreateOptions.LogContext ?? GSLogContext.Default;
        ...
        {
            if (this.ActorsManager.ActorNumberCounter == 0) // means that we did not have actors yet. this is a new game
            {
                return;
            }

            var removalTS = this.removalStartTimestamp;// we copy it here because it can changes from another thread
            if (this.ActorsManager.ActiveActorsCount == 0 &&
                (DateTime.UtcNow - removalTS).TotalMinutes > 60)
            {
                // double check if the game is still in cache: 
                if (this.GameCache.TryGetRoomReference(this.Name, null, out var room))
                {
                ...
                foreach (var actor in this.ActiveActors)
                {
                    ...
                    else
                    {
                        ...
                        if (lastTouch > this.LastTouchLimitMilliseconds)
                        {
                            ...
                            peer.Disconnect(LeaveReason.PeerLastTouchTimedout);
                    ...
                    if (currentTime - gameClientPeer.LastActivity >= inactivityInterval)
                    {
                        ...
                        Log.Warn(this.gsLogContext.InactivePeerLogGuard, msg);

                        gameClientPeer.RemovePeerFromCurrentRoom(LeaveReason.PeerLastTouchTimedout, msg);

                        try
                        {
                            gameClientPeer.Disconnect(LeaveReason.PeerLastTouchTimedout);
        ...
        protected override void OnGameFull(HivePeer peer, JoinGameRequest joinGameRequest, SendParameters sendParameters)
        {
            ...
            if (this.onGameIsFullErrorsCounter.Increment(1) >= this.ErrorsCountToInitiateUpdate)
            {
                if (this.gameUpdaterGuard.Increment(1) == 1)
                {
                    ...
                    {
                        Log.WarnFormat(this.gsLogContext.FullGameReinitLogGuard, "Game '{0}' has sent reinit message to master after getting {1} 'Game is full' errors",
