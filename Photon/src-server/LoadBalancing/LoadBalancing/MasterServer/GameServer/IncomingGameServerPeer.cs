...
namespace Photon.LoadBalancing.MasterServer.GameServer
{
    public class IncomingGameServerPeer : InboundS2SPeer, IGameServerPeer
    {
        ...
        void IGameServerPeer.DetachFromContext()
        {
            this.Context = null;

            if (log.IsDebugEnabled)
            {
                log.Debug($"Context detached from peer. p:{this}");
            }

            this.OnContextDetached();
        }

        void IGameServerPeer.WaitForDisconnect(int timeout)
        {
            this.WaitForDisconnect(timeout);
        }
        #endregion
