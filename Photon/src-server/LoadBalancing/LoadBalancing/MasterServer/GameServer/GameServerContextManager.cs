...
namespace Photon.LoadBalancing.MasterServer.GameServer
{
    public class GameServerContextManager
    {
        ...
        {
            GameServerContext[] gameServers;
            lock (this.gameServerContexts)
            {
                gameServers = this.gameServerContexts.Values.Select(x => x.Context).ToArray();
            }

            foreach (var serverContext in gameServers)
            {
                ...
                lock (this.gameServerContexts)
                {
                    serverContext.DetachPeerAndClose();

                    this.RemoveContext(serverContext, peer);
                }
                peer?.WaitForDisconnect(MasterServerSettings.Default.ConnectionDisconnectTimeout);
            }
        }
        ...
        private void ScheduleContextRemoval(GameServerContext context, IGameServerPeer peer)
        {
            ...
            if (!this.application.Running)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"Application is closing we do not schedule context removal. key:'{context.Key}', id:'{context.ServerId}',p:{peer}");
                }

                return;
            }

            if (!this.gameServerContexts.TryGetValue(context.Key, out var keeper))
            {
            ...
            {
                keeper.DisposeTimer = this.fiber.Schedule(() => this.ScheduledDestroyContext(keeper), this.contextTTL);
                if (log.IsInfoEnabled)
                {
