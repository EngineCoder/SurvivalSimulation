namespace Photon.LoadBalancing.MasterServer
{
    ...
    public class ApplicationStats
    {
        ...
        private void OnUpdateMasterPeerCount(int diff)
        {
            ...
            {
                log.DebugFormat("Updating master peer count. count:{0}, diff:{1}", this.PeerCountMaster, diff);
            }
            this.OnStatsUpdated();
        }
        ...
        private void OnStatsUpdated()
        {
            if (log.IsDebugEnabled)
            {
                log.Debug("Appstats OnStats updated");
            }
            ...
            if (this.subscribers.Count == 0)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug("Appstats OnStats updated - no subscribers");
                }
            ...
            if (this.publishSchedule != null)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug("Appstats OnStats updated - publish was already scheduled");
                }
        ...
        private void OnAddSubscriber(MasterClientPeer subscriber)
        {
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("Peer subscribed to app stats. p:{0}", subscriber);
            }
