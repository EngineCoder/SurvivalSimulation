...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class LinkedListDictionary<TKey, TValue> : IEnumerable<TValue>
    {
        ...
        public LinkedListNode<TValue> First => this.linkedList.First;

        public IEnumerable<TValue> Values => this.linkedList;
        ...
        {
#if NETCOREAPP
            if (this.dict.Remove(key, out var node) == false)
            {
                return false;
            }
...
#else
            if (this.dict.TryGetValue(key, out var node) == false)
            {
                return false;
            }
        ...
        {
            if (this.dict.TryGetValue(key, out var node))
            {
                value = node.Value;
                return true;
            }
