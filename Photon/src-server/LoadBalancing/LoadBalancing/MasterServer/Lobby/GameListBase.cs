...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    public abstract class GameListBase : IGameList
    {
        ...
        public virtual void AddGameState(GameState gameState, Dictionary<string, object> authCookie = null)
        {
            ...
            LBPerformanceCounters.MSLobbyGamesCount.Increment();
        }
        ...
        public int CheckJoinTimeOuts(DateTime minDateTime)
        {
            ...
            {
                this.RemoveGameStateFromList(gameState);
        ...
        public bool RemoveGameStateFromList(GameState game)
        {
            return this.RemoveGameStateByState(game);
        }
        ...
        public virtual bool UpdateGameState(UpdateGameEvent updateOperation, GameServerContext incomingGameServer, out GameState gameState)
        {
            ...
            bool oldVisible = gameState.IsVisibleInLobby;
        ...
        private bool RemoveGameStateByState(GameState gameState)
        {
            ...
            var removeFromGLResult = this.RemoveGameState(gameState);

            if (removeFromGLResult != removeResult)
            {
                log.ErrorFormat("Game remove results are not equal. RemovedFromApp:{0}. FromGameList:{1}, Game:{2}", 
                    removeResult, removeFromGLResult, gameId);
            }
            return removeFromGLResult;
        }
        ...
        protected virtual bool RemoveGameState(GameState gameState)
        {
            ...
            var gameId = gameState.Id;
            if (!this.gameDict.Remove(gameId))
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("game is not found in game list index. game:{0}, lobby:{1}, lobbyType:{2}", gameId, this.Lobby.LobbyName, this.Lobby.LobbyType);
                }
                return false;
            }
            ...
            LBPerformanceCounters.MSLobbyGamesCount.Decrement();
