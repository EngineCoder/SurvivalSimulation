...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class GameState
    {
        ...
        public DateTime CreateDateUtc => this.createDateUtc;
        ...
        public LinkedListNode<GameState> JoinableListNode { get; set; }
        ...
        /// </value>
        public bool HasBeenCreatedOnGameServer { get; private set; }

        /// <summary>
        /// Indicates whether client set LobbyProperties filter or not
        /// </summary>
        public bool HasLobbyPropertiesFilter { get; private set; }
        ...
        /// <summary>
        ///   Gets a value indicating whether this instance is visible in the lobby.
        /// </summary>
        /// <value>
        ///   <c>true</c> if this instance is visible in lobby; otherwise, <c>false</c>.
        /// </value>
        public bool IsVisibleInLobby => this.IsVisible && this.HasBeenCreatedOnGameServer;
        ...
        /// </summary>
        public int JoiningPlayerCount => this.joiningPeers.Count;
        ...
        /// </summary>
        public int PlayerCount => this.GameServerPlayerCount + this.InactivePlayerCount + this.JoiningPlayerCount + this.YetExpectedUsersCount;
        ...
        public Dictionary<object, object> Properties { get; private set; } = new Dictionary<object, object>();
        ...
        public string PeerCreator { get; set; }

        public DataContract LastUpdateRequest { get; set; }
        ...
        public string GetServerAddress(ILobbyPeer peer)
        {
            ...
            Func<GameServerContext, bool> filter = ctx => ctx.SupportedProtocols == null || ctx.SupportedProtocols.Contains((byte)peer.NetworkProtocol);
            ...
            else
            {
                if (!filter(this.GameServer))
                {
                    ...
                    throw new NotSupportedException(
                        $"Failed to get GS for game. It is on GS which does not support protocol {peer.NetworkProtocol} ");
                }
            }
            ...
            switch (peer.NetworkProtocol)
            {
                ...
                case NetworkProtocolType.WebSocket:
                    ...
                    // to allow containers behind LB
                    address = addrInfo.WebSocketHostname;
                    break;
                case NetworkProtocolType.SecureWebSocket:
                    address = addrInfo.SecureWebSocketHostname;
                    if (peer.LocalPort == 443)
                    {
                        address = addrInfo.SecureWebSocketHostname443;
                    }
                    break;
                ...
                default:
                    throw new NotSupportedException(
                        $"No GS address configured for Protocol {peer.NetworkProtocol} (Peer Type: {((PeerBase)peer).NetworkProtocol})");
            }
            if (string.IsNullOrEmpty(address))
            {
                throw new NotSupportedException(
                    $"No GS address configured for Protocol {peer.NetworkProtocol} (Peer Type: {peer.NetworkProtocol}, AddressFamily: {peer.LocalIPAddress.AddressFamily})");
            }

            if (log.IsDebugEnabled)
            {
                log.Debug($"Creating/joining game '{this.Id}' on server:'{address}'. ExpectedProtocol:{peer.NetworkProtocol}, p:{peer}");
            }
            return address;
        }
        ...
        public bool Update(UpdateGameEvent updateEvent)
        {
            ...
            if (updateEvent.Reinitialize)
            {
                ...
                {
                    log.DebugFormat(
                        updateEvent.Replication == 1
                            ? "Game is reinitialized. game:{0}"
                            : "Game is replicated. game:{0}", this.Id);
                }
            }
            ...
            {
                this.HasLobbyPropertiesFilter = true;
            ...
            if (updateEvent.Reinitialize)
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat(
                        updateEvent.Replication == 1
                            ? "Game reinitialization done. game:{0}"
                            : "Game replication done. game:{0}", this.Id);
                }
            }
