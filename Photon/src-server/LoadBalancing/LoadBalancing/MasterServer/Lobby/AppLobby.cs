...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class AppLobby
    {
        ...
        /// </summary>
        public int PeerCount => this.peers.Count;

        /// <summary>
        /// Gets the total number of players in all games in this lobby.
        /// </summary>
        public int PlayerCount => this.GameList.PlayerCount;

        public int GameCount => this.GameList.Count;

        #endregion

        #region Public Methods

        public void EnqueueOperation(MasterClientPeer peer, Operation operation, SendParameters sendParameters)
        {
            this.OnClientOperationEnqueued();
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.ExecuteOperation(peer, operation, sendParameters);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnExecuteOperationFinish(operation, delta);
                }
            });
        }
        ...
        {
            this.OnNonClientOperationEnqueued();
            ...
            this.ExecutionFiber.Enqueue(()=>
            {
                ...
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnEnqueuedTaskFinish(delta);
                }
            });
        }
        ...
        public void RemoveGame(GameState game)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleRemoveGameState(game);
                }
                finally
                {
                    var ticksDelta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleRemoveGameFinish(ticksDelta);
                }
            });
        }
        ...
        public void RemoveGameServer(GameServerContext gameServer)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleRemoveGameServer(gameServer);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleRemoveGameServerFinish(delta);
                }
            });
        }
        ...
        public void UpdateGameState(UpdateGameEvent operation, GameServerContext incomingGameServer)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleUpdateGameState(operation, incomingGameServer);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleUpdateGameFinish(delta);
                }
            });
        }
        ...
        public void JoinLobby(MasterClientPeer peer, JoinLobbyRequest joinLobbyRequest, SendParameters sendParameters)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleJoinLobby(peer, joinLobbyRequest, sendParameters);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleJoinLobbyFinish(delta);
                }
            });
        }
        ...
        public void LeaveLobby(MasterClientPeer peer)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleLeaveLobby(peer);
                }
                finally
                {

                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleLeaveLobbyFinish(delta);
                }
        ...
        public void ResetGameServer(GameState gameState)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleResetGameServer(gameState);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleResetGameServerFinish(delta);
                }
            });
        }
        ...
        {
            return $"Name:{this.LobbyName},Type:{this.LobbyType}, GamesCount:{this.GameCount}";
        }
        ...
        public void OnBeginReplication(GameServerContext gameServerContext)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleReplicationBegin(gameServerContext);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;

                    this.OnHandleReplicationBeginFinish(delta);
                }
            });
        }
        ...
        public void OnFinishReplication(GameServerContext gameServerContext)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleReplicationFinish(gameServerContext);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleReplicationFinishFinish(delta);
                }
            });
        }
        ...
        public void OnStopReplication(GameServerContext gameServerContext)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.HandleReplicationStop(gameServerContext);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnHandleReplicationStopFinish(delta);
                }
            });
        }
        ...
        public void UpdateLobbyLimits(bool gameListUseLegacyLobbies, int? gameListLimit, int? gameListLimitUpdates, int? gameListLimitSqlFilterResults)
        {
            ...
            this.ExecutionFiber.Enqueue(() =>
            {
                ...
                try
                {
                    this.GameList.UpdateLobbyLimits(gameListUseLegacyLobbies, gameListLimit, gameListLimitUpdates,
                            gameListLimitSqlFilterResults);
                }
                finally
                {
                    var delta = Stopwatch.GetTimestamp() - ts;
                    this.OnUpdateLobbyLimitsFinish(delta);
                }
            });
        }
        ...
        public void UpdateMatchmakingStoredProcedure(string matchmakingStoredProcedure)
        {
            if (this.LobbyType == AppLobbyType.SqlLobby)
            {
                ...
                this.ExecutionFiber.Enqueue(() =>
                {
                    ...
                    try
                    {
                        ((SqlGameList)this.GameList).UpdateMatchmakingStoredProcedure(matchmakingStoredProcedure);
                    }
                    finally
                    {
                        var delta = Stopwatch.GetTimestamp() - ts;
                        this.OnUpdateMatchmakingStoredProcedureFinish(delta);
                    }
                });
            }
        }
        ...
        protected virtual void OnClientOperationEnqueued()
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Increment();
            LBPerformanceCounters.MSLobbyClientOpsEnq.Increment();
            LBPerformanceCounters.MSLobbyClientOpsRate.Increment();
            LBPerformanceCounters.MSLobbyTaskRate.Increment();
        }

        protected virtual void OnNonClientOperationEnqueued()
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Increment();
            LBPerformanceCounters.MSLobbyTaskRate.Increment();
        }

        protected virtual void OnExecuteOperationFinish(Operation operation, long delta)
        {
            LBPerformanceCounters.MSLobbyClientOpsEnq.Decrement();
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyOpsET.Increment(delta * 1000);
        }

        protected virtual void OnEnqueuedTaskFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnHandleUpdateGameFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        ...
        protected virtual void OnHandleRemoveGameFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnHandleRemoveGameServerFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnHandleJoinLobbyFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyClientOpsEnq.Decrement();
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
        ...
        protected virtual void OnHandleLeaveLobbyFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyClientOpsEnq.Decrement();
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyOpsET.Increment(delta * 1000);
        ...
        protected virtual void OnHandleResetGameServerFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnHandleReplicationBeginFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnHandleReplicationFinishFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnHandleReplicationStopFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }
        protected virtual void OnUpdateLobbyLimitsFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void OnUpdateMatchmakingStoredProcedureFinish(long delta)
        {
            LBPerformanceCounters.MSLobbyTaskEnq.Decrement();
            LBPerformanceCounters.MSLobbyTasksET.Increment(delta * 1000);
        }

        protected virtual void ExecuteOperation(MasterClientPeer peer, Operation op, SendParameters sendParameters)
        {
            ...
            {
                op.OnStart();
                switch ((OperationCode)op.Code)
                {
                    default:
                        response = new OperationResponse(op.Code)
                        {
                    ...
                    case OperationCode.CreateGame:
                        var createGameOp = op as CreateGameRequest;
                        response = this.HandleCreateGame(peer, createGameOp);
                        break;

                    case OperationCode.JoinGame:
                        var joinGameOp = op as JoinGameRequest;
                        response = this.HandleJoinGame(peer, joinGameOp);
                        break;

                    case OperationCode.JoinRandomGame:
                        var joinRandomOp = op as JoinRandomGameRequest;
                        response = this.HandleJoinRandomGame(peer, joinRandomOp);
                        break;

                    case OperationCode.DebugGame:
                        if (peer.AllowDebugGameOperation)
                        {
                            var debugGameOp = op as DebugGameRequest;
                            response = this.HandleDebugGame(peer, debugGameOp);
                        }
                        else
                        {
                            response = new OperationResponse(op.Code)
                    ...
                    case OperationCode.GetGameList:
                        var getGameListOp = op as GetGameListRequest;
                        response = this.HandleGetGameList(peer, getGameListOp);
                        break;
                }
                op.OnComplete();
            }
        ...
        protected virtual OperationResponse HandleCreateGame(MasterClientPeer peer, CreateGameRequest operation)
        {
            ...
            // try to create game
            if (!this.TryCreateGame(operation, peer.ExpectedProtocol, false, out var gameCreated, out var gameState, out var response, peer.AuthCookie))
            {
                return response;
            }
            ...
            if (!gameState.CheckSlots(peer.UserId, operation.AddUsers, out var errMsg))
            {
                if (gameCreated)
                {
                    this.GameList.RemoveGameStateFromList(gameState);
                }
            ...
            return new OperationResponse(operation.Code, createGameResponse);
        }

        protected virtual OperationResponse HandleJoinGame(MasterClientPeer peer, JoinGameRequest operation)
        {
            ...
            else
            {
                ...
                {
                    if (!this.TryCreateGame(operation, peer.ExpectedProtocol, true, out gameCreated, out gameState, out var response, peer.AuthCookie))
                    {
                        return response;
                    }
                }
        ...
        protected virtual OperationResponse HandleJoinRandomGame(MasterClientPeer peer, JoinRandomGameRequest operation)
        {
            // try to find a match
            ...
            if (result == ErrorCode.NoMatchFound && operation.CreateIfNotExists)
            {
                ...
                {
                    operation.GameId = Guid.NewGuid().ToString();
                    operation.OperationRequest.Parameters[(byte)ParameterKey.GameId] = operation.GameId;
                }

                //replace GameProperties (used for filter) with values for game creation
                if (operation.Properties != null)
                {
                    operation.OperationRequest.Parameters[(byte)ParameterKey.GameProperties] = operation.Properties;
                }

                // validate the operation request
                var createGameOp = new CreateGameRequest(peer.Protocol, operation.OperationRequest, peer.UserId);
                if (OperationHelper.ValidateOperation(createGameOp, log, out var response) == false)
                {
                    return response;
                }

                //create - we try to use current HandleCreateGame implementation
                var createGameResponse = this.HandleCreateGame(peer, createGameOp);

                if (createGameResponse.ReturnCode != 0)
                {
                    return new OperationResponse { OperationCode = operation.Code, ReturnCode = createGameResponse.ReturnCode, DebugMessage = createGameResponse.DebugMessage };
                }
                ...
                return new OperationResponse(operation.Code, joinRandomGameResponse)
                {
            ...
            return new OperationResponse(operation.Code, joinResponse);
        }

        protected virtual OperationResponse HandleDebugGame(MasterClientPeer peer, DebugGameRequest operation)
        {
            ...
            return new OperationResponse(operation.Code, debugGameResponse);
        }

        protected virtual OperationResponse HandleGetGameList(MasterClientPeer peer, GetGameListRequest operation)
        {
            ...
            if (this.LobbyType != AppLobbyType.SqlLobby)
            {
                return new OperationResponse
                {
                    OperationCode = operation.Code,
                    ReturnCode = (short)ErrorCode.OperationInvalid,
                    DebugMessage = $"Invalid lobby type: {this.LobbyType}"
                };
            }
            ...
            return new OperationResponse(operation.Code, getGameListResponse);
        }
        ...
        private void CheckJoinTimeOuts()
        {
            ...
            LBPerformanceCounters.MSLobbyTasksET.Increment((endTS - ts) * 1000);
            LBPerformanceCounters.MSLobbyJoinTimeoutCheckET.Increment((endTS - ts) * 1000);
        }
        ...
        private void HandleRemoveGameState(GameState game)
        {
            try
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("HandleRemoveGameState: gameId={0}, joiningPlayers={1}", game.Id, game.JoiningPlayerCount);
                }

                this.GameList.RemoveGameStateFromList(game);
                this.SchedulePublishGameChanges();
            }
            catch (Exception ex)
            {
        ...
        private void PublishGameChanges()
        {
            ...
            var endTS = Stopwatch.GetTimestamp();
            LBPerformanceCounters.MSLobbyTasksET.Increment((endTS - ts) * 1000);
            LBPerformanceCounters.MSLobbyPublishGamesET.Increment((endTS - ts) * 1000);
        }
            ...
            out bool gameCreated, out GameState gameState, out OperationResponse errorResponse, Dictionary<string, object> authCookie)
        {
            ...
            if (properties != null)
            {
                if (!gameState.TrySetProperties(properties, out _, out var debugMessage))
                {
                    if (gameCreated)
                    {
