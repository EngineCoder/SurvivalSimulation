...
namespace Photon.LoadBalancing.MasterServer
{
    public class MasterClientPeer : Peer, ILobbyPeer, ICustomAuthPeer, IAuthTimeoutCheckerClient
    {
            ...
            : base(initRequest)
        {
            ...
            this.ExpectedProtocol = initRequest.NetworkProtocol == NetworkProtocolType.WebSocket && this.IsConnectionSecure 
                ? NetworkProtocolType.SecureWebSocket : initRequest.NetworkProtocol;
        ...
        public virtual OperationResponse HandleCreateGame(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            var createGameRequest = new MSCreateGameRequest(this.Protocol, operationRequest, this.UserId);
            ...
            if (!createGameRequest.ValidatePropertiesSize(this.MaxPropertiesSizePerRequest, out var errorMsg))
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Limit exceeded. create game operation ignored. errorMsg:{0}, p:{1}", errorMsg, this);
                }
                return new OperationResponse((byte)OperationCode.CreateGame)
                {
                    DebugMessage = errorMsg,
                    ReturnCode = SocketServer.ErrorCodes.OperationParametersLimitExceeded,
                };
            }
            ...
            createGameRequest.OnInserted();
            this.IncConcurrentJoinRequest(1);
            lobby.EnqueueOperation(this, createGameRequest, sendParameters);
        ...
        public virtual OperationResponse HandleJoinGame(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            var joinGameRequest = new MSJoinGameRequest(this.Protocol, operationRequest, this.UserId);
            ...
            if (!joinGameRequest.ValidatePropertiesSize(this.MaxPropertiesSizePerRequest, out var errorMsg))
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Limit exceeded. join game operation ignored. errorMsg:{0}, p:{1}", errorMsg, this);
                }
                return new OperationResponse((byte)OperationCode.CreateGame)
                {
                    DebugMessage = errorMsg,
            ...
            {
                this.IncConcurrentJoinRequest(1);
                joinGameRequest.OnInsert();
                gameState.Lobby.EnqueueOperation(this, joinGameRequest, sendParameters);
                return null;
            }
            ...
            this.IncConcurrentJoinRequest(1);
            joinGameRequest.OnInsert();
            lobby.EnqueueOperation(this, joinGameRequest, sendParameters);
        ...
        public virtual OperationResponse HandleJoinRandomGame(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            this.IncConcurrentJoinRequest(1);
            joinRandomGameRequest.OnInsert();
            lobby.EnqueueOperation(this, joinRandomGameRequest, sendParameters);
        ...
        public virtual OperationResponse HandleGetGameList(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            lobby.EnqueueOperation(this, getGameListRequest, sendParameters);
            return null;
        }
            ...
            SendParameters sendParameters, AuthSettings authSettings, OperationResponse operationResponse)
        {
            ...
            //all such responses will be send encrypted if that is possible
        ...
        private void OnAuthSuccess(AuthenticateRequest request)
        {
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("On Auth success called. p:{0}", this);
            }
            var app = (MasterApplication)ApplicationBase.Instance;
