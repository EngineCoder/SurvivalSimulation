...
namespace Photon.LoadBalancing.MasterServer
{
    public class PlayerCache : IDisposable
    {
        ...
        private static readonly LogCountGuard MaxJoinGamesGuard = new LogCountGuard(new TimeSpan(0, 1, 0), 60);
        ...
        private void HandleOnConnectedToMaster(ILobbyPeer peer)
        {
            try
            {
                ...
                {
                    var gameId = playerState.ActiveGame == null ? string.Empty : playerState.ActiveGame.Id;
        ...
        private void HandleOnDisconnectFromMaster(string playerId)
        {
            try
            {
                ...
                if (this.playerDict.TryGetValue(playerId, out var playerState) == false)
                {
                    return;
                }
        ...
        private void HandleFiendFriends(PeerBase peer, FindFriendsRequest request, SendParameters sendParameters)
        {
            try
            {
                ...
                for (var i = 0; i < request.UserList.Length; i++)
                {
                    ...
                    if (this.playerDict.TryGetValue(request.UserList[i], out var playerState))
                    {
                        onlineList[i] = true;
                        if (playerState.ActiveGame != null)
                        {
        ...
        private void LogMaxJoinedGames()
        {
            ...
            log.Warn(MaxJoinGamesGuard, $"Maximum joined games for last minute is {this.maxGamesCount} for player:{this.playerIdWithMaxGamesCount}");
