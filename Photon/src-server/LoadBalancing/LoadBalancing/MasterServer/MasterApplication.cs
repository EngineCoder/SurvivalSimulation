...
namespace Photon.LoadBalancing.MasterServer
{
    public class MasterApplication : ApplicationBase
    {
        ...
        private static IConfiguration LoadConfiguration()
        {
            ...
            var cbpath = Path.GetDirectoryName(typeof(MasterApplication).Assembly.Location);
            var c = cb.AddXmlFile(Path.Combine(cbpath, "Master.xml.config")).Build();
        ...
        protected override void OnStopRequested()
        {
            ...
            if (this.GameServers != null)
            {
                ...
                foreach (var peer in gameServers)
                {
                    if (peer != null)
                    {
                        if (log.IsDebugEnabled)
        ...
        protected override void TearDown()
        {
            ...
            this.GameServers?.ShutdownConnections();
        }
        ...
        protected virtual InboundController GetInboundController()
        {
            var controller = new InboundController(
                ...
                MasterServerSettings.Default.Limits.Inbound.Operations.BlockNonRegisteredOps);
            ...
            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Authenticate,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.AuthenticateRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.JoinLobby,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.JoinLobbyRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.LeaveLobby,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.LeaveLobbyRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.CreateGame,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.CreateGameRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.JoinGame,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.JoinGameRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.JoinRandomGame,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.JoinRandomGameRate));


            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.FindFriends,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.FindFriendsRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.LobbyStats,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.LobbyStatsRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Settings,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.SettingsRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.Rpc,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.RpcRate));

            controller.SetupOperationLimits((byte)Hive.Operations.OperationCode.GetGameList,
                new OperationLimits(InboundController.Unlimited,
                    MasterServerSettings.Default.Limits.Inbound.Operations.GetGameListRate));
            return controller;
        }
