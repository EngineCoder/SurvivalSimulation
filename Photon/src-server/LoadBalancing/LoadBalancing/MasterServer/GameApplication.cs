    ...
    public class GameApplication : IDisposable
    {
        ...
        public virtual bool TryCreateGame(string gameId, AppLobby lobby, byte maxPlayer, GameServerContext gameServer, out GameState gameState, out ErrorCode errorCode, out string errorMsg)
        {
            ...
            if (result)
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Created game: gameId={0}, appId={1}, server:{2}", gameId, this.ApplicationId, gameServer.AddressInfo.UdpAddress);
                }
            }
            else
            {
                errorCode = ErrorCode.GameIdAlreadyExists;
                errorMsg = HiveErrorMessages.GameAlreadyExist;
            }
        ...
        public void OnGameUpdateOnGameServer(UpdateGameEvent updateGameEvent, GameServerContext gameServer)
        {
            ...
            lock (this.gameDict)
            {
                if (!this.gameDict.TryGetValue(updateGameEvent.GameId, out gameState))
                {
                    ...
                    {
                        if (!this.LobbyFactory.GetOrCreateAppLobby(updateGameEvent.LobbyId, (AppLobbyType) updateGameEvent.LobbyType, out var lobby, out var errorMsg))
                        {
                        ...
                        this.GetOrCreateGame(updateGameEvent.GameId, lobby, maxPlayers, gameServer, out gameState, out var errorCode, out errorMsg);
            ...
            if (gameState != null)
            {
                ...
                gameState.LastUpdateRequest = updateGameEvent;
                gameState.LastUpdateDateTime = DateTime.UtcNow;
        ...
        public void OnGameRemovedOnGameServer(GameServerContext context, string gameId, byte removeReason)
        {
            ...
            if (found)
            {
                ...
                {
                    gameState.Lobby.RemoveGame(gameState);
                }
            }
        ...
        public bool RemoveGameByName(string gameId)
        {
            ...
            {
                log.DebugFormat(
                    removed ? "Removed game: gameId={0}, appId={1}" : "Game to remove not found: gameId={0}, appId={1}",
                    gameId, this.ApplicationId);
            }
