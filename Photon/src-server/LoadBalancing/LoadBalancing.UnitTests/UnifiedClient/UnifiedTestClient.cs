...
namespace Photon.LoadBalancing.UnifiedClient
{
    public class UnifiedTestClient : UnifiedClientBase, IAuthSchemeClient
    {
        ...
        public OperationResponse TryAuthenticate(string userId, Dictionary<byte, object> authParameter)
        {
            this.UserId = userId;
            var op = CreateAuthenticateRequest(userId);
            ...
            Assert.That(this.SendRequest(op, true), Is.True);
        ...
        {
            this.EncryptionData = (Dictionary<byte, object>)response[ParameterCode.EncryptionData];
            this.Client.SetupEncryption(this.EncryptionData);
