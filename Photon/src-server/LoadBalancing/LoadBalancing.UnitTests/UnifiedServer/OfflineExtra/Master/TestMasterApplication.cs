...
namespace Photon.LoadBalancing.UnitTests.UnifiedServer.OfflineExtra.Master
{
    ...
    public class TestMasterApplication : MasterApplication, ITestMasterApplication
    {
...
#if NETFRAMEWORK
            var cbpath = Path.GetDirectoryName(typeof(TestMasterApplication).Assembly.CodeBase.Replace("file://", ""));
#else
            var cbpath = Path.GetDirectoryName(typeof(TestMasterApplication).Assembly.Location);
        ...
        public void SetPluginConfig(string pluginConfigFileName)
        {
            Hive.Common.Settings.Default.PluginConfig = pluginConfigFileName;
        }
    }
}
