        ...
        protected override void FixtureSetup()
        {
            ...
            try
            {
                ...
                if (this.authPolicy != AuthPolicy.AuthOnMaster || this.connectPolicy.UseNSToGetMS)
                {
                    ...
                    {
                        {ParameterCode.EncryptionMode, (byte) (this.connectPolicy.Protocol == ConnectionProtocol.Udp?EncryptionModes.DatagramEncryptionGCM:EncryptionModes.PayloadEncryption)}
                    };
...
#pragma warning restore CS0618 // Type or member is obsolete
                ...
                case AuthPolicy.UseAuthOnce:
                    ...
                    if (authParameter.ContainsKey(ParameterCode.EncryptionMode) == false)
                    {
                        authParameter.Add(ParameterCode.EncryptionMode, (byte)(this.connectPolicy.Protocol == ConnectionProtocol.Udp?EncryptionModes.DatagramEncryptionGCM:EncryptionModes.PayloadEncryption)); //Encryption mode is required for UseAuthOnce
                    }
                    response = this.ConnectAndAuthenticate(client, this.NameServerAddress, authParameter);
                    this.MasterAddress = (string)response[ParameterCode.Address];
                    this.ConnectAndAuthenticate(client, this.MasterAddress, client.UserId, authParameter, reuseToken:true, expectedErrorCode);
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            if (this.authPolicy == AuthPolicy.UseAuthOnce && authParameter.ContainsKey(ParameterCode.EncryptionMode) == false)
            {
                authParameter.Add((byte)ParameterCode.EncryptionMode, (byte)(this.connectPolicy.Protocol == ConnectionProtocol.Udp ? EncryptionModes.DatagramEncryptionGCM : EncryptionModes.PayloadEncryption));
            }
