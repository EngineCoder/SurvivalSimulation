...
namespace Photon.LoadBalancing.UnitTests.UnifiedServer
{
    public class CustomAuthHttpListener : IDisposable
    {
        ...
        public void Stop()
        {
            if (log.IsDebugEnabled)
            {
                log.Debug("Http Listener stopping");
            }
            lock (this.listener)
            {
                if (this.IsDisposed)
        ...
        public void Dispose()
        {
            if (log.IsDebugEnabled)
            {
                log.Debug("Http Listener disposing");
            }
            lock (this.listener)
            {
                if (this.IsDisposed)
        ...
        private static async Task HandleRequest(HttpListenerContext context)
        {
            try
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Handling of the request, request:{0}", context.Request.RawUrl);
                }
                ...
                {
                    if (userNameParam == "testUserName" || userNameParam=="yes")
                    {
                        if (log.IsDebugEnabled)
                        {
                        ...
                        // success
                        data = Encoding.UTF8.GetBytes($"{{ \"ResultCode\": 1, \"UserId\": \"{userNameParam}\", \"Nickname\": \"nick\", \"AuthCookie\": {{ \"Param1\": 1, \"Param2\": \"2\" }}}}");
                    }
                    else
                    {
                        if (log.IsDebugEnabled)
