...
namespace Photon.LoadBalancing.UnitTests.UnifiedTests
{
    public abstract partial class LBApiTestsImpl
    {
        ...
        /// <summary>
        /// we check that EmptyRoomTTL and PlayerTTL are set from the beginning
        /// </summary>
        [Test]
        public void Props_EmptyRoomTTLAndPlayerTTLTest()
        {
            ...
            try
            {
                ...
                var r = new CreateGameRequest
                {
                    GameId = roomName,
                };

                var createResponse = client1.CreateGame(r);
                ...
                createResponse = client1.CreateGame(r);
                ...
                var props = joinResponse.GameProperties;

                Assert.That(props, Is.Not.Null);
                Assert.That(props, Does.ContainKey((byte)GameParameter.PlayerTTL));
                Assert.That(props, Does.ContainKey((byte)GameParameter.EmptyRoomTTL));
        ...
        public void Props_BroadcastProperties()
        {
            ...
            try
            {
                ...
                var roomProperties = joinResponse.GameProperties;
                Assert.IsNotNull(roomProperties);
                Assert.AreEqual(9, roomProperties.Count);

                Assert.IsNotNull(roomProperties[GamePropertyKey.IsOpen], "IsOpen");
                Assert.IsNotNull(roomProperties[GamePropertyKey.IsVisible], "IsVisible");
                Assert.IsNotNull(roomProperties[GamePropertyKey.EmptyRoomTtl], "EmptyRoomTtl");
                Assert.IsNotNull(roomProperties[GamePropertyKey.PlayerTtl], "PlayerTtl");
                Assert.IsNotNull(roomProperties[GamePropertyKey.PropsListedInLobby], "PropertiesInLobby");
                Assert.IsNotNull(roomProperties["P1"], "P1");
                Assert.IsNotNull(roomProperties["P2"], "P2");


                Assert.AreEqual(true, roomProperties[GamePropertyKey.IsOpen], "IsOpen");
                Assert.AreEqual(true, roomProperties[GamePropertyKey.IsVisible], "IsVisible");

                Assert.AreEqual(1, roomProperties[GamePropertyKey.MasterClientId], "MasterClientId");
                Assert.AreEqual(3, ((string[])roomProperties[GamePropertyKey.PropsListedInLobby]).Length, "PropertiesInLobby");
                Assert.AreEqual("L1", ((string[])roomProperties[GamePropertyKey.PropsListedInLobby])[0], "PropertiesInLobby");
                Assert.AreEqual("L2", ((string[])roomProperties[GamePropertyKey.PropsListedInLobby])[1], "PropertiesInLobby");
                Assert.AreEqual("L3", ((string[])roomProperties[GamePropertyKey.PropsListedInLobby])[2], "PropertiesInLobby");
                Assert.AreEqual(1, roomProperties["P1"], "P1");
                Assert.AreEqual(2, roomProperties["P2"], "P2");
                ...
                roomProperties = (Hashtable)ev.Parameters[ParameterCode.Properties];
                Assert.IsNotNull(roomProperties);
                Assert.AreEqual(3, roomProperties.Count);

                Assert.IsNull(roomProperties["P1"], "P1");
                Assert.IsNotNull(roomProperties["P2"], "P2");
                Assert.IsNotNull(roomProperties["P3"], "P3");

                Assert.AreEqual(null, roomProperties["P1"], "P1");
                Assert.AreEqual(20, roomProperties["P2"], "P2");
                Assert.AreEqual(3, roomProperties["P3"], "P3");
                ...
                roomProperties = (Hashtable)getPropResponse.Parameters[ParameterCode.GameProperties];
                Assert.IsNotNull(roomProperties);
                Assert.AreEqual(10, roomProperties.Count);
        ...
        public void Props_SetPlayerTTLUsingRequestParam()
        {
            ...
            try
            {
                ...
                var client2EncData = client2.EncryptionData;
                client2.LeaveGame(true);

                client2 = this.CreateMasterClientAndAuthenticate(this.Player2);
                // restore token to be able to pass auth on GS
                client2.Token = client2Token;
                client2.EncryptionData = client2EncData;
                this.ConnectAndAuthenticate(client2, gameServerAddress1, client2.UserId);
        ...
        public void Props_SetTooBigPropertiesInOneGo()
        {
            ...
            try
            {
                ...
                var createGameRequest = new CreateGameRequest
                {
                    ...
                    GameProperties = new Hashtable
                    {
                        { (byte)GameParameter.LobbyProperties, new string[]{} }
                    }
                };
                var createResponse = client1.CreateGame(createGameRequest);
        ...
        public void Props_CreatingGameWithTooBigProperties()
        {
            ...
            try
            {
                ...
                {
                    GameId = roomName,
                    GameProperties = new Hashtable { { "xxx", new byte[1_000] } }
                };
                var createResponse = client1.CreateGame(createGameRequest);
                ...
                client1.SendRequest(new OperationRequest
                {
                    ...
                    {
                        {ParameterCode.RoomName, roomName},
                        {ParameterCode.GameProperties, createGameRequest.GameProperties},

                    }
                });

                Assert.That(client1.WaitForDisconnectMessage(), Is.True);
                Assert.That(client1.Client.DisconnectError,
                    Is.EqualTo(SocketServer.ErrorCodes.SerializationLimitError));
                ...
                client1.SendRequest(new OperationRequest
                {
                    ...
                    {
                        {ParameterCode.RoomName, roomName},
                        {ParameterCode.GameProperties, new Hashtable { { "xxx", new byte[500] } }},
                        {ParameterCode.PlayerProperties, new Hashtable { { "yyy", new byte[500]}}},

                    }
                });

                var r = client1.WaitForOperationResponse();
                Assert.That(r.ReturnCode, Is.EqualTo(SocketServer.ErrorCodes.OperationParametersLimitExceeded));
                ...
                client1.SendRequest(new OperationRequest
                {
                    ...
                    {
                        {ParameterCode.RoomName, roomName},
                        {ParameterCode.PlayerProperties, new Hashtable { { "yyy", new byte[1_000]}}},
        ...
        public void Props_JoiningWithTooBigProperties()
        {
            ...
            try
            {
                ...
                client2.JoinGame(
                    ...
                    {
                        GameId = roomName,
                        GameProperties = new Hashtable { { "xxx", new byte[9_00]} }
                    }
                    , SocketServer.ErrorCodes.OperationParametersLimitExceeded);
            }
            finally
            {
                DisposeClients(client1, client2);
        ...
        [Test]
        public void Props_LobbyPropertiesCountLimit()
        {
            if (this.connectPolicy.IsOnline)
            {
                Assert.Ignore("This is an offline test");
            ...
            try
            {
                ...
                const int lobbyPropsCount = 40;
                var lobbyProps = new string[lobbyPropsCount];

                char c = 'a';
                for (int i = 0; i < lobbyPropsCount; ++i)
                {
                    lobbyProps[i] = $"{c++}";
                }
                        ...
                        {
                            {
                                (byte)ParameterKey.Properties, new Hashtable
                                {
                                    { (byte)GameParameter.LobbyProperties, lobbyProps }
                                }
                            }
                        }
                    }, 
                ...
                Assert.That(client2.Connected, Is.False);
        ...
        [Test]
        public void Props_LobbyPropertiesSizeLimit()
        {
            if (this.connectPolicy.IsOnline)
            {
                Assert.Ignore("This is an offline test");
                        ...
                        {
                            {
                                (byte)ParameterKey.Properties, new Hashtable
                                {
                                    { (byte)GameParameter.LobbyProperties, new []{"x"} },
                                    { "x", new byte[800]}
                                }
                            }
                        }
        ...
        [Test]
        public void Props_LobbyPropertiesCountLimitOnGameCreation()
        {
            if (this.connectPolicy.IsOnline)
            {
                Assert.Ignore("This is an offline test");
            ...
            try
            {
                ...
                // after creation game on master we try consequently connect to gs 
                // and create game with different parameters

                // first try with too many lobby properties in filter
                this.ConnectAndAuthenticate(client1, gameServerAddress1, client1.UserId);
                ...
                createGameRequest.GameProperties = new Hashtable
                {
                    { (byte)GameParameter.LobbyProperties, lobbyProps }
                };

                client1.CreateGame(createGameRequest, SocketServer.ErrorCodes.OperationParametersLimitExceeded);


                // second try with too many lobby properties without filter
                this.ConnectAndAuthenticate(client1, gameServerAddress1, client1.UserId);
                ...
                for (int i = 0; i < lobbyPropsCount; ++i)
                {
                    createGameRequest.GameProperties.Add($"{c++}", i);
                }
        ...
        [Test]
        public void Props_LobbyPropertiesSizeLimitOnGameCreation()
        {
            if (this.connectPolicy.IsOnline)
            {
                Assert.Ignore("This is an offline test");
        ...
        public void Props_SetTooManyUniqProperties()
        {
            ...
            try
            {
                ...
                var limitCount = GameServer.GameServerSettings.Default.Limits.Inbound.Properties.MaxUniqPropertyKeysPerPeer;
                ...
                var createGameRequest = new CreateGameRequest
                {
                    ...
                    GameProperties = new Hashtable
                    {
                        { (byte)GameParameter.EmptyRoomTTL, 123 },
                        { (byte)GameParameter.LobbyProperties, new string[]{}}
                    }// we check that correct ttl is set
                };
                var createResponse = client1.CreateGame(createGameRequest);
