    ...
    public abstract partial class LBApiTestsImpl
    {
        ...
        public void GetGameUpdate_LobbyPropertyRemove([Values(LobbyType.Default)]LobbyType lobbyType)
        {
            ...
            try
            {
                ...
                Assert.AreEqual(0, response.ReturnCode, "Expected response code 0, was: {0}. Error: {1}", response.ReturnCode, response.DebugMessage);

                Thread.Sleep(1500);

                var gameListUpdate = client2.WaitForEvent((byte)Events.EventCode.GameListUpdate, this.WaitTimeout);

                if (client2.TryWaitForEvent((byte)Events.EventCode.GameListUpdate, this.WaitTimeout, out var gameListUpdate2))
                {
                    if (gameListUpdate2 != null && gameListUpdate2.Parameters.ContainsKey((byte)ParameterKey.GameList))
                    {
