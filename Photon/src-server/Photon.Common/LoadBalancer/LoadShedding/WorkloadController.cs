...
namespace Photon.Common.LoadBalancer.LoadShedding
{
    public class WorkloadController
    {
        ...
        private AverageCounterReader peersCounter;
        ...
        public WorkloadController(
            ApplicationBase application, string instanceName, int updateIntervalInMs, string serverId, string workLoadConfigFile, int maxCcu)
        {
            try
            {
                ...
                this.feedbackControlSystem = new FeedbackControlSystem(maxCcu, application.ApplicationRootPath, workLoadConfigFile);
                this.cpuCounter.UpdateValuesHistoryLen(this.feedbackControlSystem.CPUValuesHistoryLen);
            ...
            {
                log.Error($"Exception during WorkloadController construction. Exception Msg: {e.Message}", e);
            }
        }
        ...
        public bool UseLoadPrediction => this.feedbackControlSystem?.UseLoadPrediction ?? false;

        public int PredictionAlgo => this.feedbackControlSystem?.PredictionAlgo ?? 0;
        ...
        private void InitWindowsSpecificCounters(string instanceName)
        {
            ...
            this.peersCounter = new AverageCounterReader(AverageHistoryLength, "Photon Socket Server", "Peers", instanceName);
            if (!this.peersCounter.InstanceExists)
            {
                log.WarnFormat("Did not find counter {0}", this.peersCounter.Name);
            }
        ...
        private void Update()
        {
            ...
            if (this.cpuCounter.IsValid)
            {
                // if history len was updated we update it right before next calculation
                this.cpuCounter.UpdateValuesHistoryLen(this.feedbackControlSystem.CPUValuesHistoryLen);
            ...
            if (this.peersCounter.IsValid)
            {
                var peerCount = this.peersCounter.GetNextAverage();

                this.feedbackControlSystem.SetPeerCount((int)peerCount);
            }
