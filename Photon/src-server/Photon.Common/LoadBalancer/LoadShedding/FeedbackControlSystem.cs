...
namespace Photon.Common.LoadBalancer.LoadShedding
{
    internal sealed class FeedbackControlSystem : IFeedbackControlSystem, IDisposable
    {
        ...
        public FeedbackLevel Output => this.controllerCollection.Output;

        public bool UseLoadPrediction { get; private set; }

        public int PredictionAlgo { get; private set; }

        public int CPUValuesHistoryLen { get; private set; } = 50; // default value same as FeedbackControlSystemSection, this must be > 0
        #endregion
        ...
        private static List<FeedbackController> GetNonConfigurableControllers(int maxCcu)
        {
            ...
            var peerCountThresholds = new SortedDictionary<FeedbackLevel, FeedbackLevelData>
                {
                    ...
                    {FeedbackLevel.Highest, new FeedbackLevelData(maxCcu,      maxCcu*9/10)},
                };
        ...
        private void Initialize(string workLoadConfigFile)
        {
            ...
            var filename = Path.Combine(this.applicationRootPath, workLoadConfigFile);
            
            if (!ConfigurationLoader.TryLoadFromFile(filename, out var section, out var message))
            {
                log.WarnFormat(
            ...
            if (section != null)
            {
                this.CPUValuesHistoryLen = section.CPUValuesHistoryLen;

                // load controllers from config file.);
                foreach (var controllerElement in section.FeedbackControllers)
                {
                    var dict = new SortedDictionary<FeedbackLevel, FeedbackLevelData>();
                    foreach (var level in controllerElement.Levels)
                    {
