...
namespace Photon.Common.LoadBalancer
{
    ...
    public class LoadBalancer<TServer> where TServer : IComparable<TServer>
    {
        ...
        public int AverageWorkloadPercentage
        {
            get
            {
                ...
                try
                {
                    ...
                    return (int)Math.Round((double)this.totalWorkload /
                        (this.serverCount * (int)FeedbackLevel.LEVELS_COUNT) * 100);
                }
                finally
                {
        ...
        public int TotalWorkload => this.totalWorkload;

        public int ServersInUseWeight => this.serversInUseWeight;

        public int[] LoadLevelWeights => this.loadLevelWeights;

        public FeedbackLevel PriorityDownThreshold
        {
            get => this.priorityDownThreshold;
            set => this.priorityDownThreshold = value;
        }

        public FeedbackLevel PriorityUpThreshold
        {
            get => this.priorityUpThreshold;
        ...
        /// <summary>
        /// we use it during prediction calculations
        /// </summary>
        public float PredictionSpeedFactor { get; set; }

        public float PredictionScaleFactor { get; set; }
        #endregion
        ...
        /// </param>
        /// <param name="filter">filter that can be used to select server basing on some property</param>
        /// <returns>
        ///   True if a server instance with enough remaining workload is found; otherwise false.
        /// </returns>
        public bool TryGetServer(out TServer server, Func<TServer, bool> filter)
        {
            ...
            try
            {
                ...
                server = default;// this.availableServers.First.Value.Server;
        ...
        public bool TryRemoveServer(TServer server, byte priority = 0)
        {
            ...
            try
            {
                ...
                if (bunch.TryGetServer(server, out var serverState) == false)
                {
                    return false;
                }

                bunch.RemoveServer(serverState, out var takenFromReserve);
                --this.serverCount;
        ...
        /// <param name="priority">Server priority</param>
        /// <param name="state">describes current state of server</param>
        /// <returns>
        ...
        private void InitializeFromConfig(string configFilePath)
        {
            ...
            if (!ConfigurationLoader.TryLoadFromFile(configFilePath, out var section, out var message))
            {
                log.WarnFormat(
            ...
            if (section != null)
            {
                ...
                this.PredictionScaleFactor = section.PredictionScaleFactor;
                this.PredictionSpeedFactor = section.PredictionSpeedFactor;
        ...
        {
            this.servers.TryGetValue(priority, out var output);
            if (output == null)
            {
        ...
        private void UpdateLoadBalancerState()
        {
            ...
            var newPriority = this.CalcCurrentPriority(out var bunchLoadLevel, out var restLoadLevel);
