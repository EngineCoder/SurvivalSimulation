...
// </copyright>
// <summary>
//   Defines the ClientAuthenticationQueue type.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace Photon.Common.Authentication.CustomAuthentication
{
    ...
    public interface IClientAuthenticationQueue
    {
        ...
        Dictionary<string, Dictionary<string, string>> CustomHttpHeaders { get; }
        ...
        void EnqueueRequest(string clientQueryStringParamters, byte[] postData, string contentType,
            Action<AsyncHttpResponse, IClientAuthenticationQueue> callback, object state, bool checkUrl = true, Dictionary<string, string> customHttpHeaders = null);
    ...
    public class ClientAuthenticationQueue : IClientAuthenticationQueue
    {
        ...
        public struct CreateParam
        {
            ...
            public Dictionary<string, Dictionary<string, string>> CustomHttpHeaders { get; set; }
        ...
        public ClientAuthenticationQueue(CreateParam param, bool checkUrl = true)
        {
            ...
            this.CustomHttpHeaders = param.CustomHttpHeaders;
        }
        ...
        {
            this.fiber.Enqueue(() => this.ExecuteRequest(clientQueryStringParameters, postData, contentType, callback, state, checkUrl, customHttpHeaders));
        }
        ...
        private void ExecuteRequest(string clientAuthenticationRequestUrl, byte[] postData, string contentType, Action<AsyncHttpResponse, ClientAuthenticationQueue> callback, object state, bool checkUrl = true, Dictionary<string, string> customHttpHeaders = null)
        {
            try
            {
                ...
                {
                    foreach (var kv in customHttpHeaders)
                    {
                        try
                        {
                            if (!ApplyRestrictedHeader(kv.Key, kv.Value, webRequest))
                            {
                                ...
                                if (log.IsDebugEnabled)
                                {
                                    log.DebugFormat("Custom header added: '{0}' '{1}'", kv.Key, kv.Value);
                                }
                            }
                        }
                        catch (Exception e)
                        {
                            log.Warn($"Custom header add exception:'{kv.Key}' with value '{kv.Value}'. Exception Msg:{e.Message}", e);
                        }
                    }
                }
            ...
            AsyncHttpRequest result, object userState, Action<AsyncHttpResponse, ClientAuthenticationQueue> userCallback)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    userCallback(
                        new AsyncHttpResponse(HttpRequestQueueResultCode.Error, this.RejectIfUnavailable,
                            userState), this);
                });
                result.Dispose();
            }
        ...
        private bool ApplyRestrictedHeader(string key, string value, HttpWebRequest webRequest)
        {
            switch (key)
            {
                ...
                default:
                    ...
                    {
                        log.WarnFormat("Usage of restricted http header :'{0}' with value '{1}'.", key, value);
                        break;
                    }
                    return false;
            }
