...
namespace Photon.NameServer
{
    public class PhotonEndpointInfo
    {
        public PhotonEndpointInfo(Node nodeInfo)
        {
            ...
            if (string.IsNullOrEmpty(nodeInfo.Region))
            {
                throw new InvalidDataException("'Region' can not be neither null nor empty. set something meaning full like 'EU' and use it during authentication");
            }
