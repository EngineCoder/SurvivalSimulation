...
namespace Photon.NameServer
{
    public class PhotonApp : ApplicationBase
    {
...
#if NETFRAMEWORK
            var cbpath = Path.GetDirectoryName(typeof(PhotonApp).Assembly.CodeBase.Replace("file://", ""));
#else
            var cbpath = Path.GetDirectoryName(typeof(PhotonApp).Assembly.Location);

#endif
            var c = cb.AddXmlFile(Path.Combine(cbpath, "NameServer.xml.config")).Build();
        ...
        protected virtual bool ReadNameServerConfigurationFile(out string message)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("Reading Name Server config file from {0}", filename);
            }
