    ...
    public class ClientPeer : Photon.SocketServer.ClientPeer, ICustomAuthPeer, IAuthTimeoutCheckerClient
    {
        ...
        protected override void OnOperationRequest(OperationRequest operationRequest, SendParameters sendParameters)
        {
            try
            {
                ...
                {
                    //all responses will be send encrypted if that is possible
                    sendParameters.Encrypted = this.IsPayloadEncryptionEnabled;
        ...
        protected OperationResponse HandleAuthenticateRequest(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            return this.HandleAuthenticateRequest(authenticateRequest, sendParameters, this.ExpectedProtocol);
        }

        protected virtual OperationResponse HandleAuthenticateRequest(AuthenticateRequest authenticateRequest,
            SendParameters sendParameters, NetworkProtocolType authOnceRequestedProtocol)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.DebugFormat( "HandleAuthenticateRequest for App ID {0} - {1}, expectedProtocol {2} authOnceRequestedProtocol={3}", authenticateRequest.ApplicationId, this, this.ExpectedProtocol, authOnceRequestedProtocol);
            }
            ...
            if (++this.NumberAuthRequests > Settings.Default.MaxNumberAuthRequests)
            {
            ...
            try
            {
                masterEndPoint = masterServer.GetEndPoint(authOnceRequestedProtocol, this.LocalPort,
               isIPv6: this.LocalIPAddressIsIPv6, useHostnames: this.IsIPv6ToIPv4Bridged);
            }
            catch (Exception e)
            {
                //webrtc
                masterEndPoint = masterServer.GetEndPoint(authOnceRequestedProtocol, 0);

                var str =
                    $"Handle Auth: Exception during GetEndPoint call. EndPoint protocol:{authOnceRequestedProtocol}, LocalPort:{this.LocalPort}, isIpV6:{this.LocalIPAddressIsIPv6}, useHostNames:{this.IsIPv6ToIPv4Bridged}";
        ...
        private OperationResponse HandleGetRegionListRequest(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            // authenticate application id
            if (!this.application.ServerCache.TryGetRegions(regionListRequest, this.ExpectedProtocol, this.LocalPort, this.LocalIPAddressIsIPv6, 
        ...
        public NetworkProtocolType ExpectedProtocol 
        {
            get
            {
                if (this.NetworkProtocol == NetworkProtocolType.WebSocket && this.IsConnectionSecure)
                {
                    return NetworkProtocolType.SecureWebSocket;
                }
        ...
        protected virtual void DoCustomAuthenticationResult(CustomAuthenticationResult customAuthResult, IAuthenticateRequest authenticateRequest, SendParameters sendParameters, AuthSettings authSettings)
        {
            ...
            try
            {
                ...
                {
                    default:
                        var returnCode = (short)ErrorCode.CustomAuthenticationFailed;

                        if (customAuthResult.CustomAuthErrorCode.HasValue)
                        {
                            switch (customAuthResult.CustomAuthErrorCode.Value)
                            {
                                //only allow these here defined return codes instead of CustomAuthenticationFailed
                                case (short)ErrorCode.NotMinimumVersion:
                                case (short)ErrorCode.InvalidVersionClient:
                                case (short)ErrorCode.InvalidVersionDashboard:
                                    returnCode = customAuthResult.CustomAuthErrorCode.Value;
                                    break;
                            }
                        }
        ...
        protected virtual void HandleCustomAuthenticateResult(CustomAuthenticationResult customAuthResult, IAuthenticateRequest authenticateRequest, SendParameters sendParameters, AuthSettings authSettings)
        {
            ...
            var expectedOrRequestedProtocol = isAuthOnceUsed ? (NetworkProtocolType)((AuthOnceRequest)nameServerAuthRequest).Protocol : this.ExpectedProtocol;
            string masterEndPoint;
            //tmp webrtc
            try
            {
                masterEndPoint = masterServer.GetEndPoint(expectedOrRequestedProtocol, this.LocalPort,
                isIPv6: this.LocalIPAddressIsIPv6, useHostnames: this.IsIPv6ToIPv4Bridged);
            }
            catch (Exception e)
            {
                masterEndPoint = masterServer.GetEndPoint(expectedOrRequestedProtocol, 0);

                var str =
                    $"Custom Auth: Exception during GetEndPoint call. EndPoint protocol:{expectedOrRequestedProtocol}, LocalPort:{this.LocalPort}, isIpV6:{this.LocalIPAddressIsIPv6}, useHostNames:{this.IsIPv6ToIPv4Bridged}";
